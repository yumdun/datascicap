...
def get_pie_chart(site):
    ...
    if site == 'ALL':
        ...
        fig = px.pie(df1, values='class count', names='class', title=f"Success Rate for site {site}")
        return fig
# TASK 4:
...
def scatter(site,payload):
    ...
    if site=='ALL':
        fig=px.scatter(dfa,x='Payload Mass (kg)',y='class',color='Booster Version Category',title='Success Count by Payload Mass for all sites')
        return fig
    else:
        dfb=dfa[dfa['Launch Site']==site]
        fig=px.scatter(dfb,x='Payload Mass (kg)',y='class',color='Booster Version Category',title=f'Success Count by Payload Mass for site {site}')
        return fig
